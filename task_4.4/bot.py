...
from database import create_database, add_message, select_n_last_messages, create_table, insert_row, insert_row_tts
...
def start(message):
    ...
    bot.send_message(message.chat.id,
                     text= f"Привет, {user_name}! Отправь мне голосовое сообщение или текст, и я тебе отвечу!\n"
                            ...
                            f"Выбери кнопку /stt, чтобы проверить распознавание речи",
...
def stt(message):
    ...
    if status_stt:
        ...
        if not stt_text:
            ...
            bot.send_message(user_id, "Кажется, проверка распознавания речи прошла удачно, теперь введи любое сообщение")
            bot.register_next_step_handler(message, start)
    else:
        bot.send_message(user_id, "При работе SpeechKit произошла ошибка")
...
def tts(message):
    ...
    if error_message:
        bot.send_message(user_id, error_message)
        return
    print("tts_symbols = is_tts_symbol_limit(user_id, text) = ", tts_symbols)

    # Запись ответа GPT в БД
    # БД: добавляем сообщение пользователя и его роль в базу данных
    full_user_message = [message.text, 'user', 0, tts_symbols, 0]
    ...
    bot.send_message(user_id, "Кажется, проверка синтеза речи прошла удачно, теперь введи любое сообщение")
    bot.register_next_step_handler(message, start)
